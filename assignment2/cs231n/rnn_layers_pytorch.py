...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    no_stimulatd_h = x @ Wx + prev_h @ Wh + b  #对于张量点乘要用@
    next_h = torch.tanh(no_stimulatd_h)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h = torch.zeros((N, T, H))  # 可以理解为把解析图片的步骤分为T次，然后每次都对上个扫描的图片信息进行记忆，这个记忆计就是prev_h的本质
    prev_h = h0
    
    for t in range(T):   # T是总时间步，是RNN的关键，决定要为图片生成几个单词
        next_h = rnn_step_forward(x[:, t, :], prev_h, Wx, Wh, b)  # 每次扫描的输出都受前几次扫描输出的记忆影响，使后续单词的生成更具逻辑性
        h[:, t, :] = next_h
        prev_h = next_h
...
def word_embedding_forward(x, W):
    ...
    """
    # W是词汇表的权重系数，V是训练后模型的词库，D是每个词的特征向量，比如是否为动物，是否可爱，是否常见等
    # x作为整个模型的第一个输入，是单词序列，而图片的特征只参与h0的初始化
    out = W[x]  # 二维数组的二维索引，会根据索引数组的形状寻找W中的各行，即各个词特征向量 最后得到(N,T,D)即每个样本的每个时间步对应一个词向量
