...
def affine_backward(dout, cache):
    ...
    dx = dout.dot(w.T).reshape(x.shape)  #变回三维形状，
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var #理解为训练出一个用于缩放数据的模型
...
def batchnorm_backward(dout, cache):
    ...
    dvar = dx_hat * np.sum(-0.5 * inv_std ** 3 * x_minus_mean, axis=0)   # dL/dx = dL/dvar * dvar/dx
    dvar_dx = 2 * inv_N * x_minus_mean    # dL/dvar = dL/dx_hat * dx_hat/dvar ，配凑就看哪些变量表达式里包括了该变量

    dmean_from_var = dvar * np.sum(-2 * inv_N * x_minus_mean, axis=0) # dL/dx = dL/dmean * dmean/dx
